...
def get_location():
    ...
    locations = {
        "México": "Pabellón M, Monterrey NL",
...
def insert_scan_record(attendee: dict, event_id):
    ...
    record = ExhibitorScan(
        ...
        scanned_by_rep_name=session.get("scanned_by_rep_name"),
    ...
    if channel:
        publish_records_event(
            channel, {"type": "record_created", "e_scan_id": record.e_scan_id}
        )
...
def process_exhibitor_scan():
    ...
    if event is None:
        return (
            jsonify(
                {
                    "result": False,
                    "message": "No hay evento activo para registrar el escaneo",
                }
            ),
            400,
        )
    ...
    if day not in (3, 4):
        return (
            jsonify(
                {
                    "result": False,
                    "message": f"Sólo puedes escanear Contactos entre {(event.start_date + timedelta(days=2)).strftime('%d/%m/%Y')} y {(event.end_date).strftime('%d/%m/%Y')}",
                }
            ),
            400,
        )
    ...
    record = (
        ...
        .filter(
            ...
            ExhibitorScan.scanned_a_last_name
            == attendee.get("scanned_a_last_name", ""),
    ...
    if channel and record:
        publish_records_event(channel, {"type": "record_created", "record": record})
...
def update_exhibitor_record_notes():
    if not is_exhibitor_edit_window(g.get("active_event")):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Evento Finalizado. Sólo puedes consultar y exportar contactos.",
                }
            ),
            403,
        )
    ...
    if record:
        if record.notes == notes:
            return jsonify(
                {"success": True, "message": "No se realizaron cambios en las notas"}
            )
        record.notes = notes
        db.session.commit()
        ...
        if channel:
            publish_records_event(
                channel, {"type": "record_updated", "record": record.to_dict()}
            )
        return jsonify({"success": True, "message": "Notas guardadas exitosamente"})
    else:
        return jsonify(
            {"success": False, "message": "No se pudieron guardar las notas"}
        )
...
def add_or_update_appointment():
    ...
    if appointment:
        if (
            str(appointment.date) == date
            and str(appointment.hour) == hour
            and str(appointment.description) == description
        ):
            return jsonify(
                {
                    "message": "No se realizaron cambios en la cita agendada",
        ...
        channel = build_records_channel(
            current_user.company, appointment.exhibitor_scan.event_id
        )
        if channel:
            publish_records_event(
                channel,
                {
                    "type": "record_updated",
                    "record": appointment.exhibitor_scan.to_dict(),
                },
            )
        return jsonify(
            {
    ...
    if scan_record:
        ...
        if channel:
            publish_records_event(
                channel, {"type": "record_updated", "record": scan_record.to_dict()}
            )

    return jsonify(
        {"message": "Cita agendada correctamente", "appointment": new_appt.to_dict()}
    )
...
END:VCALENDAR"""
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment;filename=cita_{appointment_id}.ics"
        },
    )
