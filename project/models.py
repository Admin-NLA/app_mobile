...
class Event(db.Model):
    ...
    stats_ev = db.relationship(
        "Stats", back_populates="event", uselist=False, cascade="all, delete-orphan"
    )
...
class Stats(db.Model):
    ...
    event_id = db.Column(
        db.Integer, db.ForeignKey("events.event_id"), unique=True, nullable=False
    )
...
class ExhibitorScan(db.Model):
    ...
    scanned_by_rep_name = db.Column(db.String(255))
...
class Appointment(db.Model):
    ...
    e_scan_id = db.Column(
        db.Integer,
        db.ForeignKey("exhibitors_scans.e_scan_id"),
        unique=True,
        nullable=False,
    )
